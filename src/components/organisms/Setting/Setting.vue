<template>
  <CardModal>
    <div slot="header" class="is-marginless">
      <base-header :level="3" class="has-text-left"> {{ id ? 'Edit' : 'Create' }} Tutorial </base-header>
    </div>
    <div slot="body">
      <validatable-text-field label="Name" v-model="innerName" placeholder="First timers" name="name" rules="required"> </validatable-text-field>
      <validatable-textarea-field label="Description" v-model="innerDescription" placeholder="Tutorial for first time customers." name="description"> </validatable-textarea-field>
      <div>
        <div class="has-padding-bottom-4">
          Start this tutorial for a user visiting the following url.
        </div>
        <columns v-if="innerPath.regex" class="path-regex-value">
          <div>
            {{ origin }}
          </div>
          <div>
            <text-field :value="innerPath.value" @input="updateInnerPath('value', $event)" name="regex" :rules="innerPath.regex ? 'required|' : ''"></text-field>
          </div>
          <div>
        ...
        <columns v-else>
          <column>
            <textarea-field row="3" :value="origin + path.value + formatParameters(innerParameters)" disabled></textarea-field>
          </column>
        </columns>
        <columns v-if="innerPath.value !== '/'">
          <column>
            <base-check-box :value="innerPath.regex" @input="updateInnerPath('regex', $event)">
        ...
        <columns>
          <column>
            <base-check-box v-model="withParameters">
              with parameters
            </base-check-box>
          </column>
        </columns>
        <parameter-fields v-show="withParameters" v-model="innerParameters"> </parameter-fields>
      </div>
    </div>
    <div slot="footer" class="has-margin-0">
      <base-button @click="onSaveClick" is-primary>
        Save
      </base-button>
      <base-button @click="onCancelClick" class="is-text">
        Cancel
      </base-button>
    </div>
  </CardModal>
