...
import { SET_USER, SELECT_TUTORIAL, SET_SERVER_SIDE_ERRORS, SYNC_DATA, SORT_TUTORIALS, SET_REQUESTING } from './mutation-types';
...
const mutations = {
  ...
  [SYNC_DATA](state, payload) {
    ...
    state.tutorials = payload.tutorials.map(tutorial => new TutorialEntity(tutorial));
    if (stateTutorialLength === 0 && payloadTutorialLength === 1 && !state.selectedTutorialID) {
...
const getters = {
  tutorial(state) {
    return state.selectedTutorialID ? state.tutorials.find(t => t.id === state.selectedTutorialID) : null;
  },
};
