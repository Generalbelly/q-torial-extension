import firebase, { FieldValue, Timestamp, convertDocumentsToArray, convertDocToObject } from '../../firebase';
import { ADD_TUTORIAL, UPDATE_TUTORIAL, DELETE_TUTORIAL, UPDATE_SEARCH_QUERY, UPDATE_ORDER_BY, SET_REQUESTING, SET_ALL_FETCHED, RESET_STATE } from '../mutation-types';
...
const mutations = {
  ...
  [UPDATE_TUTORIAL](state, payload) {
    ...
    state.tutorials = [...state.tutorials.slice(0, index), tutorial, ...state.tutorials.slice(index + 1)];
  },
  [DELETE_TUTORIAL](state, payload) {
    const { id = null } = payload;
    const index = state.tutorials.findIndex(tutorial => tutorial.id === id);
    if (index === -1) return;
    state.tutorials = [...state.tutorials.slice(0, index), ...state.tutorials.slice(index + 1)];
  },
};
...
const actions = {
  listTutorials: ({ state, commit }, payload) => {
    const { searchQuery = null, userID = null, orderBy = ['createdAt', 'desc'] } = payload;
    ...
    if (latestSnapshot && latestSnapshot.docs.length > 0) {
      snapshot = snapshot.startAfter(latestSnapshot.docs[latestSnapshot.docs.length - 1]);
    }
