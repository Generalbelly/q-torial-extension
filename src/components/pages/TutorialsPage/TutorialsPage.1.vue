...
export default {
  ...
  computed: {
    ...mapState('tutorial', ['projectEntity', 'tutorialEntities', 'selectedTutorialId', 'selectedStepId', 'requestState', 'projectNotFound']),
    ...mapState(['extLog']),
    ...mapGetters('tutorial', ['selectedTutorial', 'selectedStep', 'isRequesting']),
  },
  created() {
    this.listTutorials();
  },
  methods: {
    ...mapActions('tutorial', ['listTutorials', 'addTutorial', 'updateTutorial', 'deleteTutorial', 'selectTutorial', 'selectStep', 'addStep', 'updateStep', 'deleteStep']),
    ...mapActions(['retrieveLog', 'saveLog']),
    onClickSave(data) {
      const { id = null, steps = [], parameters = [], path = {}, name, description } = data;
      if (id) {
        this.updateTutorial({
          data: {
            id,
